...
const reducer: Reducer<SessionState> = (
  ...
  switch (action.type) {
    case actionTypes.START_SESSION_SUCCESS:
      return startSessionSuccess(state, action);
    case actionTypes.DELETE_SESSION_SUCCESS:
      return deleteSessionSuccess(state, action);
    case actionTypes.CHANGE_SESSION_FAILURE_NUM_SUCCESS:
      return changeFailureRateSuccess(state, action);
    case actionTypes.SET_FAILURE_RATE_NUMBER:
      return failureRateNumber(state, action);
    default:
      return state;
  }
};
